...
async function orderCreate(req,res) {
    ...
    try {
        ...
        const order_id = insertOrder.shift(); //id + cantidad de inserciones (POSICION CERO PORQUE QUIERO SOLO EL ID)
        ...
        try {
            ...
            const totalOrderPrice = await totalPriceResponse(orderTableJoin,order_id) // funcion que devuelve objeto con detalle de la orden
            const total_or = parseInt (totalOrderPrice.totalOrder[0].total_orden); //total de la orden. Extraigo number porque la base toma formato int
